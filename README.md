...
- Third Party
  ...
  - Firebase
- 기간
  - 2023.09.13 ~ 2023.09.27 (2주) +
    2023.10.11 ~ 2023.10.17 (1주)
    총 3주간
...
- 2023.10.11
  ...
  - 진행 해야할 목표
    ...
    - 서버에 접속중 확인중 -> 버그 수정 - check
    - 플레이어 죽으면 관전으로 진행
    - 캐릭터 선택 추가
      - 캐릭터 화살표 추가
    - 공격시 화면 흔들림 효과 -> API(시네마신 카메라 등)

  - 진행된 목표

    - 호스트, 게스트에서 각 몬스터가 같은 플레이어를 타겟팅 하며 따라간다.
    - 보스를 처치할 경우 모든 클라이언트는 엔딩으로 넘어간다.
    - 멀티 : 준비 완료 된 플레이어를 각 클라이언트에서 확인 가능

  - 불필요한 코드들, 클래스들 정리 및 GameManager 책임 증가

- 2023.10.12

  - 할 것들

    - 플레이어 사망시 관전으로 진행 - check
    - 플레이어 사망시 다른 플레이어로 타겟 변경 (적) - check
    - 사격시 화면 흔들림 효과 -> 시네머신 - check

    - 자신의 캐릭터를 확인하는 화살표 개선 - check
    - 설정 추가 - check
      - 전체화면모드 / 창모드 변경 추가
    - 캐릭터 추가 - check
      - 색깔만 다르고 같은 기능의 캐릭터 추가
    - 적에게 맞을 경우 흔들리는 효과로 변경 - check
    - 플레이어가 피격 당할경우 화면 주변이 어두워지는 효과 - check
